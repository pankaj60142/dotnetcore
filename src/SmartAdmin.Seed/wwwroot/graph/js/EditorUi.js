 ...
 * screen widths <= 640px.
 */
EditorUi.prototype.hsplitPosition = (screen.width <= 640) ? 118 : ((urlParams['sidebar-entries'] != 'large') ? 273 : 240);
...
EditorUi.prototype.createDivs = function () {
    ...
    this.sidebarFooterContainer = this.createSidebarFooterContainer(this);
 ...
 */
EditorUi.prototype.createSidebarFooterContainer = function (editor) {

    var div = document.createElement('div');

    var html = '<div class="geSidebarContainer geSidebarFooter" style="position: absolute; overflow: hidden; left: 0px; width: 100%; height: 38px; bottom: 0px;"><a class="geTitle" style="color: Green; font-weight: bold; height: 100%; padding-top: 9px;" ><span style="font-size: 18px; margin-right: 5px;">+</span>More Shapes...</a></div>';

    div.innerHTML = html;

    div.onclick = function () { OpenSidebarDialog(editor) };

    return div;
};

function OpenSidebarDialog(editor) {

    var dlg = new DiagramShapesDialog(editor);
    editor.showDialog(dlg.container, 800, 600, false, true);
...
EditorUi.prototype.createUi = function () {
    ...
    if (this.menubar != null) {
        ...
        this.shareContainer = this.createShareContainer();
        this.menubar.container.appendChild(this.shareContainer);
...
EditorUi.prototype.createShareContainer = function () {
    ...
    button.innerHTML = '<i class="fa fa-share-alt"></i> Share';
    ...
    div.style.cssText = "display: inline-block;padding-right: 0px;position: absolute;right: 0px;margin-top: 0px;";
    // div.innerHTML = '<button type="button" class="btn" ><i class="fa fa-share-alt"></i> Share</button>';
...
function OpenShareDialog(ui) {



    var dlg = new AddShareViaEmailDialog(ui);
    ui.showDialog(dlg.container, 420, 120, true, true);
}
...
EditorUi.prototype.saveFile = function (forceDialog, SelectedFolderId) {
    ...
    if (!forceDialog && this.editor.filename != null) {
        ...
        $.ajax({
            ...
            type: "POST",
            url: rootUrl + '/Graph/SaveData/',
            data: { "xml": xml, "fileName": this.editor.getOrCreateFilename().replace(".xml", ""), "compid": localStorage.getItem("companyid"), "updateMode": 1, "SelectedFolderId": SelectedFolderId },
            error: function (jqXHR, textStatus, errorThrown) {
                alert(errorThrown);
                swal('Alert!', 'Unable to save file', 'error');
            },
        ...
        var dlg = new FilenameDialog(this, this.editor.getOrCreateFilename(), mxResources.get('save'), mxUtils.bind(this, function (name) {
            ...
            $.ajax({
                ...
                type: "POST",
                url: rootUrl + '/Graph/SaveData/',
                data: { "xml": xml, "fileName": name.replace(".xml", ""), "compid": localStorage.getItem("companyid"), "updateMode": 0, "SelectedFolderId": SelectedFolderId },
                error: function (jqXHR, textStatus, errorThrown) {
                    alert(errorThrown);
                    swal('Alert!', 'Unable to save file', 'error');
                },
                success: function (data) {
                    ...
                    if (data.Message != "success") {
                        ...
                        edi.filename = name;
                        swal("Success", "File saved successfully", "success");
                        window.parent.document.getElementById("graphEditor").innerHTML = " (" + name.replace(".xml", "") + ")";
                        //window.parent.ReloadApplications();
